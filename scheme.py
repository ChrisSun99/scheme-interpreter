...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals.__len__() != vals.__len__():
            raise SchemeError('Wrong number of values: expected: {0}, actual:{1}'.format(formals.__len__(), vals.__len__()))
        def define_list(frame, formals, vals):
            if formals.second == nil:
                frame.define(formals.first, vals.first)
            else:
                frame.define(formals.first, vals.first)
                define_list(frame, formals.second, vals.second)
        if formals.__len__() != 0 and vals.__len__() !=0:
            define_list(child, formals, vals)
        # END PROBLEM 11
        return child
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        function = do_lambda_form(Pair(target.second, expressions.second), env)
...
#    if scheme_eval(expressions.first, env) == nil:
#        return False
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        bond = bindings.first
        check_form(bond, 2,2)
        formals = Pair(bond.first, formals)
        vals = Pair(scheme_eval(bond.second.first, env), vals)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and  scheme_symbolp(target.first):
        ...
        check_formals(formals)
        body = expressions.second
        function = MacroProcedure(formals, body, env)
        env.define(target.first, function)
        return target.first
    else:
        raise SchemeError
